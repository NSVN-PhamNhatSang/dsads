...
/**
 * The Enum EmployeeReferenceRangeImport.
 */
//社員参照範囲
public enum EmployeeReferenceRangeImport {
	...
	/** The Constant values. */
	private final static EmployeeReferenceRangeImport[] values = EmployeeReferenceRangeImport.values();
	 ...
	 */
	private EmployeeReferenceRangeImport(int value, String nameId, String description) {
		this.value = value;
		this.nameId = nameId;
	 ...
	 */
	public static EmployeeReferenceRangeImport valueOf(Integer value) {
		// Invalid object.
		if (value == null) {
			return null;
		}
		...
		// Find value.
		for (EmployeeReferenceRangeImport val : EmployeeReferenceRangeImport.values) {
			if (val.value == value) {
				return val;
			}
		}

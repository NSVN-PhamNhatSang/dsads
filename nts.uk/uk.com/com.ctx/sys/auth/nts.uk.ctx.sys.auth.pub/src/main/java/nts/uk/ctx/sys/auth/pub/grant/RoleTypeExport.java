...
public enum RoleTypeExport {
	...
	/** The Constant values. */
	private final static RoleTypeExport[] values = RoleTypeExport.values();
	 ...
	 */
	private RoleTypeExport(int value, String nameId, String description) {
		this.value = value;
		this.nameId = nameId;
	 ...
	 */
	public static RoleTypeExport valueOf(Integer value) {
		// Invalid object.
		if (value == null) {
			return null;
		}
		...
		// Find value.
		for (RoleTypeExport val : RoleTypeExport.values) {
			if (val.value == value) {
				return val;
			}
		}

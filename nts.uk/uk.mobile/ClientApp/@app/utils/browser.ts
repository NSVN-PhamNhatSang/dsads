    ...
    public static get private() {
        return new Promise((resolve) => {
            const _w = window as any,
                yes = () => resolve(true), // is in private mode
                not = () => resolve(false), // not in private mode
                testLocalStorage = () => {
                    try {
            ...
            // Chrome & Opera
            if (_w.webkitRequestFileSystem) {
                return void _w.webkitRequestFileSystem(0, 0, not, yes);
            }
            // Firefox
            ...
            if (isSafari) {
                ...
                try {
                    _w.openDatabase(null, null, null, null);
            ...
            // IE10+ & Edge InPrivate
            if (!_w.indexedDB && (_w.PointerEvent || _w.MSPointerEvent)) {
                return yes();
            }
